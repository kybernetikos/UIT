var Promise = (function(){
	...
	mixin(Promise.prototype, {
		...
		then: function(onResolve, onReject) {
			...
			function whenResolved() {
				...
				if (typeof onResolve == 'function') {
					result = onResolve.apply(null, Array.prototype.slice.call(arguments));
				}
				if (result === undefined) {
					pipelined.resolve.apply(pipelined, Array.prototype.slice.call(arguments));
				} else {
			...
			function whenRejected() {
				...
				if (typeof onReject === 'function') {
					result = onReject.apply(null, Array.prototype.slice.call(arguments));
				}
				if (result === undefined) {
					pipelined.reject.apply(pipelined, Array.prototype.slice.call(arguments));
				} else {
		...
		apply: function(resolveFunc, rejectFunc) {
			return this.then( rejectFunc === undefined ? undefined :
				function() {
					return Promise.wrap(resolveFunc.apply(null, arguments));
				}, rejectFunc === undefined ? undefined : 
				function() {
					return Promise.wrap(rejectFunc.apply(null, arguments));
				}
			);
		},
	...
	mixin(Promise, {
		...
		all: function() {
			...
			for (var i = 0, len = args.length; i < len; ++i) {
				(function(i) {
					...
					promise.then(
							function() {
								values[i] = Array.prototype.slice.call(arguments);
								if (--waitingFor < 1) {
        ...
        lift: function(func) {
            return function() {
                return Promise.wrap( [ func.apply(null, arguments) ] );
            }
        },
